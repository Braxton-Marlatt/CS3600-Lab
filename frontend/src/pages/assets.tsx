...
const STATUS = {'not-started': "Active", 'in-repair': "In Repair", 'finished': "Retired"}
...
function StatusBadge({status, icon=false}: { status: 'not-started' | 'in-repair' | 'finished', icon?: boolean}) {
  ...
  return <span className={`inline-flex items-center justify-center text-xs font-medium h-6 w-6 ${icon ? "": "px-2.5 py-0.5 w-auto"} rounded ${cls}`}>{icon ? STATUS_ICON[status] : STATUS[status]}</span>
}
    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        >
          ...
          {Object.entries(STATUS).map(([status, label], i) => (
            <option key={i} value={status}>{label}</option>
          ))}
        </select>
        <Link
          to="/add-asset"
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-5 py-3 border-t flex justify-end gap-2">
              ...
              <button onClick={() => { setSelected(null); setEditSelected(selected); }}
      ...
      {editSelected && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center px-5 py-4 border-b">
              <h5 className="font-semibold text-gray-800">Edit Asset</h5>
            ...
            <div className="px-5 py-4">
              <table className="w-full text-sm">
                <tbody>
                {[
                  ['Asset ID', editSelected.id],
                  ['Name', editSelected.name],
                  ['Category', editSelected.category],
                  ['Location', editSelected.location],
                  ['Assigned Employee', editSelected.assignedEmployee ?? '—'],
                  ['Assigned Department', editSelected.assignedDepartment ?? '—'],
                  ['Status', editSelected.status],
                  ['Purchase Date', editSelected.purchaseDate],
                ].map(([k, v]) => (
                  <tr key={k} className="border-b border-gray-100 last:border-0">
                    <th className="py-2 pr-4 text-left font-medium text-gray-600 w-32">{k}:</th>
                    <td className="py-2 text-gray-800">{k === 'Status' ?
            ...
            <div className="px-5 py-3 border-t flex justify-end gap-2">
              ...
              <button className="text-sm bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">Save Changes
              </button>
            </div>
          </div>
        </div>
